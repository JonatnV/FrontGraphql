...
function DynamicQuery() {
  ...
  const [selectedBreedFields, setSelectedBreedFields] = useState(["id","name"]);
  const [selectedStudentFields, setSelectedStudentFields] = useState(["id","name","career"]);
  const [breedId, setBreedId] = useState("siam");
  ...
  return (
    <div style={{padding:20}}>
      <div>
        <label><input type="radio" checked={mode==="students"} onChange={()=>setMode("students")} /> Students</label>
        <label style={{marginLeft:10}}><input type="radio" checked={mode==="breed"} onChange={()=>setMode("breed")} /> Cat Breed</label>
      </div>

      {mode === "breed" && (
        <div style={{marginTop:10}}>
          <div>ID de raza: <input value={breedId} onChange={e=>setBreedId(e.target.value)} /></div>
          <div style={{marginTop:8}}>
            {breedFields.map(f=>(
              <label key={f} style={{display:"inline-block", marginRight:8}}>
                <input type="checkbox" checked={selectedBreedFields.includes(f)} onChange={e=>{
      ...
      {mode === "students" && (
        <div style={{marginTop:10}}>
          {studentFields.map(f=>(
            <label key={f} style={{display:"inline-block", marginRight:8}}>
              <input type="checkbox" checked={selectedStudentFields.includes(f)} onChange={e=>{
